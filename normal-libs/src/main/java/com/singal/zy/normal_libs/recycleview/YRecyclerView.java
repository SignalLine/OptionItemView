package com.singal.zy.normal_libs.recycleview;
...
public class YRecyclerView extends RecyclerView {
    ...
    private YItemDecoration mItemDecoration;
    ...
    public YRecyclerView(Context context, AttributeSet attrs){
        ...
        /*================== 获取自定义属性 begin ==================*/
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.YRecyclerView);
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.YRecyclerView_rv_type) {
                type = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.YRecyclerView_rv_orientation) {
                orientation = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.YRecyclerView_rv_column) {
                column = typedArray.getInt(attr, 1);
            } else if (attr == R.styleable.YRecyclerView_rv_divider_size) {
                dividerSize = (int) typedArray.getDimension(attr, 0f);
            } else if (attr == R.styleable.YRecyclerView_rv_divider_drawable) {
                dividerDrawable = typedArray.getDrawable(attr);
            } else if (attr == R.styleable.YRecyclerView_rv_divider_color) {
                dividerColor = typedArray.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.YRecyclerView_rv_default_animator_open) {
...
//1、设置RecyclerView的类型和方向
        ...
        mItemDecoration = new YItemDecoration(mContext, orientation, dividerSize, dividerColor, dividerDrawable);
    ...
    class RecyclerViewListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (move && newState == RecyclerView.SCROLL_STATE_IDLE) {
                ...
                if (0 <= n && n < YRecyclerView.this.getChildCount()) {
                    int top = YRecyclerView.this.getChildAt(n).getTop();
                    YRecyclerView.this.smoothScrollBy(0, top);
                }
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (move) {
                ...
                if (0 <= n && n < YRecyclerView.this.getChildCount()) {
                    int top = YRecyclerView.this.getChildAt(n).getTop();
                    YRecyclerView.this.scrollBy(0, top);
                }
            }
        }
    }
    ...
    /**
             ...
             */
    class YItemDecoration extends ItemDecoration {
        private Context mContext;
        ...
        public YItemDecoration(Context context, int orientation, int dividerSize, int dividerColor, Drawable dividerDrawable) {
            mContext = context;
            ...
            if (dividerDrawable == null) {
                ...
                if (mDividerSize == 0) {
                    if (mOrientation == YRecyclerView.ORIENTATION_VERTICAL) {

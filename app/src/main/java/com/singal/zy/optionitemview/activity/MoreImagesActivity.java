...
import com.singal.zy.optionitemview.adapter.MoreImageAdapter;
...
public class MoreImagesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mButton = (Button) findViewById(R.id.query);
                        ...
                        .subscribe(new Action1<Permission>() {
                            ...
                            public void call(Permission permission) {
                                if (permission.granted) {
                                    ...
                                    //拒绝
                                    Toast.makeText(MoreImagesActivity.this, "您拒绝了打开相机的权限，无法完成", Toast.LENGTH_SHORT).show();
                                } else {
                                    // gotoSetting();
                                    Toast.makeText(MoreImagesActivity.this, "您拒绝了打开相机的权限，无法完成", Toast.LENGTH_SHORT).show();
                                }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Toast.makeText(MoreImagesActivity.this, "请从相册选择", Toast.LENGTH_SHORT).show();
                }
            }
        }
    }
                    ...
                    public void call(Permission permission) {
                        if (permission.granted) {
                            ...
                            new Thread(new Runnable() {
                                @Override
                                public void run() {
                                    List<MoreImageBean> list2 = getContentProvider(uri, projection, orderBy);//到时候抽取接口
                                    Log.e("list", "call: " + list2.toString() + ".size" + list2.size());
                                    if (list2 != null) {
                                        if (list2.size() > 7) {//这里看要求最多几张照片
                                            list2 = list2.subList(list2.size() - 7, list2.size());
                                        }
                                        final List<MoreImageBean> finalList = list2;
                                        gridView.post(new Runnable() {
                                            @Override
                                            public void run() {
                                                //TODO 拿到数据源然后拓展
                                                gridView.setAdapter(new MoreImageAdapter(MoreImagesActivity.this, finalList));//拿到了就可以搞了
                            ...
                            //拒绝
                            Toast.makeText(MoreImagesActivity.this, "您拒绝了读取照片的权限", Toast.LENGTH_SHORT).show();
    ...
    private List<MoreImageBean> getContentProvider(Uri uri, String[] projection, String orderBy) {
        List<MoreImageBean> lists = new ArrayList<>();
...
//                Bitmap bitmap = convertToBitmap(strings, 300, 300);
                ...
                MoreImageBean myBitmap = new MoreImageBean(strings);
                lists.add(myBitmap);
            } catch (Exception e) {

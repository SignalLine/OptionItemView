package com.singal.zy.optionitemview.model.js;
...
public class NativeModule extends JsModule {
    ...
    public boolean setMenu(String title, final JBCallback callback) {
        if (getContext() != null && getContext() instanceof JSBaseActivity) {
            ((JSBaseActivity) getContext()).setMenu(title, new Runnable() {
                @Override
                public void run() {
    ...
    public void getLocation(final JBCallback callback, final JBCallback change) {
        if (getContext() != null && getContext() instanceof JSBaseActivity) {
            Location location = ((JSBaseActivity) getContext()).getLocation(new LocationListener() {
                @Override
